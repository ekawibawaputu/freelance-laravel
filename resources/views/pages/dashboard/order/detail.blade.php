                                        ...
                                        <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                                            <div class="flex mt-2 flex-nowrap">
                                                ...
                                                @forelse ($thumbnail as $item)
                                                    <img :class="{'border-4 border-serv-button': active === {{$item->id}}}" src="{{ url(Storage::url($item->thumbnail)) }}" alt="thumbnail service" @click="changeThumbnail( '{{ url(Storage::url($item->thumbnail)) }}', {{ $item->id }})" class="inline-block w-24 mr-2 rounded-lg cursor-pointer">
                                                @empty
                                                    {{-- empty --}}
                                                @endforelse
